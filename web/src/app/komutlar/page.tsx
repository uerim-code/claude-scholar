...
const categories = ["Tumu", "Arastirma", "Gelistirme", "SuperClaude"];
...
export default function CommandsPage() {
  ...
  const filtered = commands.filter((c) => {
    ...
    const matchesCategory = activeCategory === "Tumu" || c.category === activeCategory;
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-[var(--text-secondary)]">
          {commands.length} komut - Calistir butonuna tiklayin, ciktiyi alt panelde goruntuleyin.
        </p>
      </div>
      ...
      {Object.entries(grouped).map(([category, categoryCommands]) => (
        <div key={category}>
          <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
            ...
            {category}
            <span className="text-xs text-[var(--text-muted)] font-normal">({categoryCommands.length})</span>
          </h2>
          <div className="space-y-2 mb-8">
            {categoryCommands.map((cmd) => (
              <div key={cmd.id} className="card-hover bg-[var(--bg-card)] rounded-lg border border-[var(--border)] px-5 py-4 flex items-center gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    <span className="font-mono text-sm text-[var(--accent)] font-medium">{cmd.command}</span>
                    <span className="text-sm font-medium text-[var(--text-primary)]">{cmd.name}</span>
                  </div>
                  <p className="text-xs text-[var(--text-secondary)]">{cmd.description}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    title="Kopyala"
                  >
                    {copiedId === cmd.id ? <Check size={14} className="text-[var(--success)]" /> : <Copy size={14} />}
                  </button>
                  <RunButton command={cmd.command} label={cmd.name} variant="primary" size="sm" />
                </div>
              </div>
            ))}
          </div>
