    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-[var(--text-secondary)]">
          {skills.length} yetenek - Araştırma, yazım, geliştirme ve daha fazlası
        </p>
      </div>
          ...
          <div key={category}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
              {categorySkills.map((skill) => (
                <div
                  key={skill.id}
                  className="card-hover bg-[var(--bg-card)] rounded-xl border border-[var(--border)] p-5 flex flex-col"
                >
                  <div className="flex items-start justify-between mb-3">
                    <span className="text-2xl">{skill.icon}</span>
                    <button
                      className="p-2 rounded-lg bg-[var(--accent-light)] text-[var(--accent)] hover:bg-[var(--accent)] hover:text-white transition-all cursor-pointer"
                      title="Yeteneği Çalıştır"
                    >
                      <Play size={14} />
                    </button>
                  </div>
